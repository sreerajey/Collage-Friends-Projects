...
<section class="inner-banner py-5">
        <div class="w3l-breadcrumb py-lg-5">
            <div class="container pt-4 pb-sm-4">
                <h4 class="inner-text-title pt-5">Admin - All New Orders List</h4>
    ...
    <div class="w3l-grids-block-5 home-course-bg py-5" id="courses">
        <div class="container py-md-5 py-4">
            <div class="title-main text-center mx-auto mb-md-5 mb-4" style="max-width:500px;">
                <p class="text-uppercase">All User Booking Details</p>
...
$qry = "SELECT `payment`.*,`tb_product`.*,`cart`.* FROM `cart`,`tb_product`,`payment`,`tb_user_register`
WHERE `payment`.`pdid`=`cart`.`cid` AND `cart`.uid=`tb_user_register`.`ureg_id` AND `cart`.`pid`=`tb_product`.`productcode` AND `payment`.`puid`=`cart`.`uid` AND `payment`.`type`='user'";
...
$result = mysqli_query($conn, $qry);
      if ($result->num_rows > 0) {
                    ...
                    <div class="coursecard-single">
                        ...
                        <div class="content-main-top">
                            ...
                            <div class="top-content-border d-flex align-items-center justify-content-between mt-4 pt-4">
                                ...
                                if($row['token']=='no'){          
               echo "
               <a href='createtoken.php?ids=" . $row['payid'] . " '>  
            ...
            Verify And Create Token</button ></a>";
             
                                }else{
                                    echo "                                 
                                    <button type='submit'  class='btn btn-info btn-lg'  style='padding-left: 2.5rem; radius:240px; padding-right: 2.5rem;background-color: #14b38b;float:right' name='login'>
