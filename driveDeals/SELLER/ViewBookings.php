...
<section class="ftco-section ftco-no-pt bg-light">
    <div class="container">
        ...
        <div class="row">
            <?php
            $res = mysqli_query($conn, "SELECT * FROM `book`,`car`,`user` WHERE `book`.`cid`=`car`.`cid` AND `book`.`uid` = `user`.`uid` AND `car`.`cstatus` ='request'");
                    ...
                    <div class="car-wrap rounded ftco-animate">
                        ...
                        <div class="text">
                            ...
                            <p class="d-flex mb-0 d-block">
                                <a href="VerifyBooking.php?id=<?php echo $cid ?>&status=not paid" class="btn btn-primary py-2 mr-1">Approve</a>
                                <a href="VerifyBooking.php?id=<?php echo $rs['cid'] ?>&status=rejected" class="btn btn-danger py-2 ml-1">Reject</a>
                            </p>
                        </div>
                    </div>
                </div>
