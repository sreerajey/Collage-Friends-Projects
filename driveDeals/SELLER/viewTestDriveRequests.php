...
<?php
$a = "SELECT `test_drive`.*,`user`.`uname`,`car`.`cname`,`car`.`cmodel` FROM `test_drive`,`user`,`car` WHERE `car`.`cid`=`test_drive`.`cid` AND `user`.`uid`=`test_drive`.`uid`";
                        ...
                        <td>
                            ...
                            ?>
                                <div style="display: flex;justify-content: center;align-items: center;">
                                    <a href="manageRequests.php?testid=<?php echo $row['tid'] ?>&status=Accepted" class="btn btn-outline-primary ml-2">Approve</a>
                                    <a href="manageRequests.php?testid=<?php echo $row['tid'] ?>&status=Rejected" class="btn btn-outline-danger ml-2">Reject</a>
                                </div>
                            <?php } else { ?>
